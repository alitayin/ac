...
export async function GET(
  ...
  try {
    const response = await fetch(
      `${ETOKENDB_UPSTREAM_BASE_URL}/project-info-invoices/${encodeURIComponent(invoiceId)}`,
      {
        cache: "no-store",
    ...
    console.error(`Failed to proxy etokendb project info invoice ${invoiceId}:`, error)
    ...
    return NextResponse.json(
      {
        ...
        error:
          error instanceof Error
            ? error.message
            : "Failed to fetch etokendb project info invoice",
      },
      {
        status: 502,
        headers: RESPONSE_HEADERS,
