...
import { isBlockedTokenId } from './blocked-tokens.ts';
...
function formatDisplayAmountForBuy(amount, decimals) {
...
function logOrderExecutionFailure(order, buyConfig, result) {
  const payload = {
    orderKey: order?.key,
    tokenId: order?.tokenId,
    requestedAmount: order?.remainingAmount,
    requestedMaxPrice: order?.maxPrice,
    requestedTokenCostCapXec: order?.tokenCostCapXec,
    executionMaxPrice: buyConfig?.maxPrice,
    executionTokenCostCapXec: buyConfig?.tokenCostCapXec,
    reason: result?.reason || 'UNKNOWN',
    message: result?.message || 'Unknown buy execution failure',
    details: result?.details || null,
  };

  console.warn('[Auto.js] Buy execution failed', payload);

  try {
    console.warn(
      '[Auto.js] Buy execution failed JSON',
  ...
  processingPromise = (async () => {
    try {
      ...
      const ordersList = Object.entries(orders).map(([key, value]) => {
        ...
        order.buyerAddress === currentAddress && !isBlockedTokenId(order.tokenId)
      );
      ...
      for (const tokenId in ordersByToken) {
        ...
        const tokenDecimals = Number.isFinite(tokenInfo.decimals)
          ? Math.max(0, Math.trunc(tokenInfo.decimals))
          : 0;
        ...
        const buyConfig = {
          ...
          tokenDecimals,
          amount: formatDisplayAmountForBuy(
            orderToProcess.remainingAmount,
            tokenDecimals,
          ),
          maxPrice: orderToProcess.maxPrice,
          tokenCostCapXec: orderToProcess.tokenCostCapXec,
          serviceCreditEnabled: orderToProcess.serviceCreditEnabled,
        ...
        try {
          ...
          if (!result.success) {
            logOrderExecutionFailure(orderToProcess, buyConfig, result);
            if (Array.isArray(result.serviceCreditRedemptions) && result.serviceCreditRedemptions.length > 0) {
              const previousTransactions = Array.isArray(orderToProcess.transactions)
                ? orderToProcess.transactions
                : [];
              orderToProcess.transactions = [
                ...previousTransactions,
                {
                  txid: result.serviceCreditRedemptions[0]?.txid || '',
                  amount: 0,
                  networkFee: 0,
                  swapFee: 0,
                  serviceCreditUsed: result.serviceCreditUsed || 0,
                  serviceCreditRedemptions: result.serviceCreditRedemptions,
                  totalFees: 0,
                  failedAfterServiceCreditRedemption: true,
              ...
              orders[orderToProcess.key] = {
                ...
                failureReason: result.message || result.reason || 'Service credit redeemed but swap failed',
              };
              localStorage.setItem('swap_orders', JSON.stringify(orders));
              dispatchOrdersUpdated('processed');
              void queueOrdersSync(orders, orderToProcess.buyerAddress);
            }
          ...
          const remainingAmountAfterFill = Math.max(
            0,
            orderToProcess.remainingAmount - totalExecutedAmount,
          );
          orderToProcess.remainingAmount = Number(
            remainingAmountAfterFill.toFixed(tokenDecimals),
          );
          ...
          const updatedOrderData = {
            ...
            transactions: orderToProcess.transactions,
            serviceCreditEnabled: orderToProcess.serviceCreditEnabled,
            ...(typeof orderToProcess.tokenCostCapXec === 'number'
              ? { tokenCostCapXec: Math.max(
                  0,
                  orderToProcess.tokenCostCapXec -
                    executedTransactions.reduce(
                      (sum, tx) => sum + (Number(tx.tokenCost) || 0),
                      0,
                    ),
                ) }
              : {})
